...
from pcl.loader import ImageFolderInstance, TwoCropsTransform
import pcl.loader
import pcl.builder
...
parser.add_argument('--moco-k', default=16384, type=int, help='queue size; number of negative keys (default: 16384)')
parser.add_argument('--moco-m', default=0.999, type=float, help='momentum of updating key encoder (default: 0.999)')
parser.add_argument('--moco-t', default=0.2, type=float, help='softmax temperature (default: 0.2)')
...
def main():
    ...
    args.num_cluster = [int(c) for c in args.num_cluster.split(',')]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print("MoCo forward signature:", inspect.signature(model.forward))
    ...
    if args.distributed:
        ...
        print("Warning: Running without DistributedDataParallel. Proceeding on single GPU or CPU.")
    else:
    ...
    traindir = "/content/drive/My Drive/dataset_cropped/train"
    train_dataset = ImageFolderInstance(traindir,transform=TwoCropsTransform(transforms.Compose(augmentation)))
    evaldir = "/content/drive/My Drive/dataset_cropped/val"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        cluster_result = None
        print(f"[DEBUG] Epoch {epoch} | warmup_epoch = {args.warmup_epoch} | Clustering will run: {epoch >= args.warmup_epoch}")
...
def train(train_loader, model, criterion, optimizer, epoch, args, cluster_result=None):
    ...
    for i, (images, index) in enumerate(train_loader):
        ...
        if output_proto is not None:
            valid_mask = (target_proto >= 0) & (target_proto < output_proto.size(1))
            if valid_mask.any():
                loss_proto = criterion(output_proto[valid_mask], target_proto[valid_mask])
                accp = accuracy(output_proto[valid_mask], target_proto[valid_mask])[0]
            else:
                loss_proto = torch.tensor(0.0, device=output_proto.device)
                accp = torch.tensor(0.0, device=output_proto.device)
        else:
            loss_proto = torch.tensor(0.0, device=output.device)
            accp = torch.tensor(0.0, device=output.device)


        # Total loss = InfoNCE + ProtoNCE
        loss = loss + loss_proto

        # Update meters
        losses.update(loss.item(), images[0].size(0))
        acc_inst.update(accuracy(output, target)[0].item(), images[0].size(0))
...
def compute_features(eval_loader, model, args):
    ...
    for i, (images, index) in enumerate(tqdm(eval_loader)):
        with torch.no_grad():
            ...
            feat = model.encoder_q(images)  # use only query encoder
            features[index] = feat
...
def run_kmeans(x, args):
    ...
    for seed, num_cluster in enumerate(args.num_cluster):
        ...
        index = faiss.IndexFlatL2(d)  # CPU version of FAISS
...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k=None, is_eval=False, cluster_result=None, index=None):
        ...
        if cluster_result is not None:
            ...
            # If it's a list-of-lists → build a flat mapping
            if isinstance(im2cluster, list):
                import numpy as np
                flat_map = -1 * np.ones(max(index).item() + 1, dtype=np.int64)
                for cluster_id, members in enumerate(im2cluster):
                    for m in members:
                        flat_map[m] = cluster_id
                im2cluster = torch.tensor(flat_map, dtype=torch.long, device=self.device)
            elif not torch.is_tensor(im2cluster):
            ...
            # If centroids is a list of tensors/arrays, stack them
            if isinstance(centroids, list):
                centroids = centroids[0]
            elif not torch.is_tensor(centroids):
                import numpy as np
                centroids = torch.tensor(np.array(centroids), dtype=torch.float32)

            centroids = centroids.to(self.device)   # [num_clusters, dim]
            centroids = nn.functional.normalize(centroids, dim=1)
            proto_logits = torch.matmul(q, centroids.T)   # [batch, num_clusters]

            output_proto = proto_logits
